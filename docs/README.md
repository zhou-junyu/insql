...
本示例将创建一个公用的 MySqlDbContext，在各个 Service 中都将用此 MySqlDbContext。

```csharp
public class MySqlDbContext<TInsql> : DbContext where TInsql : class
{
    public MySqlDbContext(MySqlDbContextOptions<TInsql> options) : base(options)
    {
    }
    ...
    protected override void OnConfiguring(DbContextOptions options)
    {
        ...
        options.UseMySql("Server=localhost;Database=example;Uid=root;Pwd=root;");
    }
}

public class MySqlDbContextOptions<TInsql> : DbContextOptions<MySqlDbContext<TInsql>> where TInsql : class
{
    public MySqlDbContextOptions(IServiceProvider serviceProvider) : base(serviceProvider)
    {
    }
}
```
...
public void ConfigureServices(IServiceCollection services)
{
    ...
    //注册公用MySqlDbContext
    //生成周期为Scoped，如果一次请求会注入多次MySqlDbContext<UserService>，则它只会创建一个实例。
    services.AddScoped(typeof(MySqlDbContext<>));
    services.AddSingleton(typeof(MySqlDbContextOptions<>));
}
```

`UserService.cs`,`BookService.cs`
...
public class UserService : IUserService
{
    ...
    //MySqlDbContext<T> T 指定为 UserService,目的为与UserService.insql.xml type 对应
    public UserService(MySqlDbContext<UserService> dbContext)
    {
        this.dbContext = dbContext;
    }
    ...
    public UserInfo GetUser(int userId)
    {
        return this.dbContext.Query("GetUser",new { userId }).SingleOrDefault();
    }
}
...
public class BookService : IBookService
{
    ...
    //MySqlDbContext<T> T 指定为 BookService,目的为与BookService.insql.xml type 对应
    public BookService(MySqlDbContext<BookService> dbContext)
    {
        this.dbContext = dbContext;
    }
    ...
    {
        return this.dbContext.Query("GetBook",new { bookId }).SingleOrDefault();
    }
}
```

`UserService.insql.xml`,`BookService.insql.xml`

```xml
<!--与MySqlDbContext<T>中的T类型对应-->
<insql type="Insql.Tests.Domain.Services.UserService,Insql.Tests" >
  <!--定义UserInfo类型数据库字段到对象属性映射-->
  <map type="Insql.Tests.Domain.Models.UserInfo,Insql.Tests">
    <key name="user_id" to="UserId" />
...
<!--与MySqlDbContext<T>中的T类型对应-->
<insql type="Insql.Tests.Domain.Services.BookService,Insql.Tests" >
  <map type="Insql.Tests.Domain.Models.BookInfo,Insql.Tests">
    <key name="book_id" to="BookId" />
    <column name="book_author" to="BookAuthor" />
  </map>
  <select id="GetBook">
